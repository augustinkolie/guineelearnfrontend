    ...
    useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                ...
                setError("Impossible de charger le profil pour la bibliothèque.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    if (isLoading) {
        return (
            <div className="h-screen w-full flex flex-col items-center justify-center bg-white gap-4">
                <LoaderIcon className="w-12 h-12 text-[#1B6B3A] animate-spin" />
                <p className="text-[#0F2D1E] font-bold animate-pulse">Ouverture de la bibliothèque...</p>
            </div>
        );
    }
