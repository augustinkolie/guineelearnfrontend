...
export default function DashboardPage() {
    ...
    if (isLoading) {
        return (
            <div className="h-screen w-full flex flex-col items-center justify-center bg-white gap-4">
                ...
                <p className="text-[#0F2D1E] font-bold animate-pulse">Chargement de votre espace...</p>
            </div>
        );
    }
    ...
    return (
        <DashboardLayout user={user}>
            {user.role === 'STUDENT' && <StudentView user={user} profile={profile} />}
            {user.role === 'TEACHER' && <TeacherView user={user} profile={profile} />}
            {user.role === 'PARENT' && <ParentView user={user} profile={profile} />}
        </DashboardLayout>
    );
}
