        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-6 py-3 bg-white border border-gray-100 text-[#0F2D1E] rounded-xl font-bold hover:bg-gray-50 transition-all shadow-sm">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="space-y-6">
                        ...
                        <nav className="space-y-2">
                            {['Tous', 'Sécurité', 'Contenu', 'Utilisateurs', 'Système'].map((item) => (
                                <button 
                                    ...
                                    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all font-bold text-sm ${filter === item ? 'bg-[#1B6B3A] text-white shadow-lg shadow-[#1B6B3A]/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
                                >
                                    <span>{item}</span>
                    ...
                    <div className="bg-[#0F2D1E] p-6 rounded-2xl shadow-xl text-white space-y-4">
                        <div className="flex items-center gap-3">
                ...
                <div className="lg:col-span-3 space-y-4">
                    ...
                    <div className="relative mb-6">
                        ...
                        <input 
                            ...
                            className="w-full pl-12 pr-6 py-4 bg-white border border-gray-100 rounded-2xl outline-none focus:border-[#1B6B3A]/30 focus:shadow-xl focus:shadow-[#1B6B3A]/5 transition-all font-medium text-[#0F2D1E]"
                        />
                    </div>
                    ...
                    <div className="space-y-4">
                        {mockLogs.map((log) => (
                            <div 
                                key={log.id} 
                                className={`flex items-start gap-4 p-5 rounded-2xl border transition-all hover:scale-[1.01] hover:shadow-lg hover:shadow-gray-200/50 bg-white group ${getStatusColor(log.status)}`}
                            >
                                ...
                                <div className="hidden group-hover:flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button className="text-[10px] font-black text-[#1B6B3A] uppercase tracking-widest bg-white border border-gray-100 px-3 py-1.5 rounded-lg hover:bg-gray-50">Détails</button>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <button className="w-full py-4 border-2 border-dashed border-gray-200 rounded-2xl text-xs font-black text-gray-400 uppercase tracking-widest hover:border-[#1B6B3A]/30 hover:text-[#1B6B3A] transition-all group">
